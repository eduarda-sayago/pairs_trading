...
class ForecastingTrader:
    ...
    def apply_MLP(self, X, y, validation_data, test_data, n_in, hidden_nodes, epochs, optimizer, loss_fct,
        ...
        if len(history.history['loss']) < 500:
            train_score = [min(history.history['loss']), min(history.history['mae'])]
            val_score = [min(history.history['val_loss']),min(history.history['val_mae'])]
        else:
            train_score = [history.history['loss'][-1], history.history['mae'][-1]]
            val_score = [history.history['val_loss'][-1], history.history['val_mae'][-1]]
    ...
    def apply_RNN(self, X, y, validation_data, test_data, hidden_nodes, epochs, optimizer, loss_fct,
        ...
        if len(hidden_nodes) == 1:
            ...
            model.add(LSTM(hidden_nodes[0], input_shape=(X.shape[1], 1), kernel_initializer=glorot_init))
        else:
            for i in range(len(hidden_nodes)-1):
                if i == 0:
                    #model.add(LSTM(hidden_nodes[0], activation='relu', input_shape=(X.shape[1], 1),
                    #              return_sequences=True, kernel_initializer=glorot_init))
                    model.add(LSTM(hidden_nodes[0], input_shape=(X.shape[1], 1),
                                   return_sequences=True, kernel_initializer=glorot_init))
                else:
                    #model.add(LSTM(hidden_nodes[i], activation='relu', return_sequences=True,
                    #               kernel_initializer=glorot_init))
                    model.add(LSTM(hidden_nodes[i], return_sequences=True,
                                   kernel_initializer=glorot_init))
            ...
            model.add(LSTM(hidden_nodes[-1], kernel_initializer=glorot_init))# last layer does not return sequences
        # add regularization
        #model.add(Dropout(0.1))
        ...
        model.compile(optimizer=optimizer, loss=loss_fct, metrics=['mae'])
        model.summary()
        plot_model(model, to_file='./rnn_models/model.png', show_shapes=True,
        ...
        if len(history.history['loss']) < 20:
            ...
            train_score = [history.history['loss'][-1], history.history['mae'][-1] if history.history['mae'] else 0]
            val_score = [history.history['val_loss'][-1], history.history['val_mae'][-1]]
